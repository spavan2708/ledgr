  ...
  if (!plan) {
    return (
      <div className="mx-auto mt-20 max-w-4xl text-center">
        ...
        <p className="mt-4 text-slate-400">Please complete your financial profile.</p>
        <Link href="/profile" className="mt-6 inline-block primary-button">Go to Profile</Link>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto w-full max-w-5xl py-10">
      <header className="mb-10 text-center">
        <h1 className="text-3xl font-bold tracking-tight text-white sm:text-5xl">Your Financial Plan</h1>
        <p className="mt-4 text-sm leading-6 text-slate-400">Educational simulation based on deterministic rules.</p>
      </header>
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        ...
        <div className="space-y-8">
          ...
          <Card title="Monthly Cash Flow">
            ...
            <Row label="Discretionary Expenses" value={`- ${formatRupees(calc.totalMonthlyExpenses - calc.totalEssentialExpenses - debt.debtPaymentRatio * calc.totalMonthlyIncome / 100)}`} />
            <Row label="Debt Payments" value={`- ${formatRupees(calc.totalMonthlyIncome * debt.debtPaymentRatio / 100)}`} />
            ...
            {!emergency.isSufficient && (
              <div className="mt-4 rounded-xl bg-amber-400/10 p-4">
                <p className="text-sm text-amber-200">
                  ...
                  <br />We recommend directing {formatRupees(emergency.recommendedMonthlyContribution)} / month here before investing elsewhere.
                </p>
              </div>
            )}
          </Card>
          ...
          {/* 5. Risk Profile */}
          <Card title="Preliminary Risk Profile">
            <div className="mb-4 flex items-center justify-between border-b border-white/5 pb-4">
              <span className="text-2xl font-bold text-white">{risk.category}</span>
              <span className="rounded-full bg-emerald-400/10 px-3 py-1 text-xs font-bold text-emerald-300">Score: {risk.score}</span>
            </div>
        ...
        <div className="space-y-8">
          ...
          <Card title="Asset Allocation">
            <p className="mb-6 text-sm text-slate-400">Comparing your current investable assets to a {risk.category.toLowerCase()} target profile.</p>
            ...
            <div className="space-y-4">
              ...
              {Object.entries(allocation.targetAllocation).map(([key, target]) => {
                const current = allocation.currentAllocation[key];
                const stat = allocation.comparison[key].status;
                const name = key === "cash" ? "Cash/Bank" : key === "fd" ? "Fixed Deposits" : key === "mutual_funds" ? "Mutual Funds" : key === "stocks" ? "Stocks/Equity" : key === "bonds" ? "Bonds/Debt" : "Gold";
                
                return (
                  <div key={key} className="grid grid-cols-12 items-center gap-2 border-t border-white/5 pt-3 text-sm">
                    <div className="col-span-4 font-medium text-slate-300">{name}</div>
                    <div className="col-span-3 text-right text-white">{formatPercent(current)}</div>
                    <div className="col-span-3 text-right text-slate-400">{formatPercent(target)}</div>
                    <div className="col-span-2 text-right">
                      {stat === "Overweight" ? (
                        <span className="text-amber-400 text-xs">Over</span>
                      ) : stat === "Underweight" ? (
                        <span className="text-sky-400 text-xs">Under</span>
                      ) : (
          ...
          {/* 7. Recommended Investment */}
          <Card title="Recommended Monthly Plan">
            <div className="mb-6">
              <p className="text-sm text-slate-400">Total investable amount available</p>
              <p className="text-3xl font-bold text-emerald-400">{formatRupees(recs.investableMonthlyAmount)}</p>
            </div>
            
            {recs.investableMonthlyAmount > 0 ? (
              <div className="space-y-3">
                <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Allocation Strategy for New Funds</p>
                {Object.entries(recs.recommendedAllocation).map(([key, amount]) => {
                  if (amount <= 0) return null;
                  const name = key === "mutual_funds" ? "Mutual Fund SIP" : key === "fd" ? "Fixed Deposits" : key === "stocks" ? "Stocks/Equity" : key === "bonds" ? "Bonds/Debt" : key === "cash" ? "Cash Savings" : "Gold";
                  return (
                    <div key={key} className="flex justify-between border-b border-white/5 py-2 text-sm">
                      <span className="text-slate-300">{name}</span>
                      <span className="font-bold text-white">{formatRupees(amount)}</span>
                    </div>
                  );
                })}
                <p className="mt-4 text-xs leading-5 text-slate-500">
                  Note: SIP is an investment method used to automatically distribute these funds monthly. This represents new deployment, not selling existing assets.
                </p>
              </div>
            ) : (
              <div className="rounded-xl bg-slate-800/50 p-4">
                <p className="text-sm text-slate-400">You currently have no surplus available for new investments. Focus on managing expenses and debt.</p>
              </div>
            )}
          </Card>
              ...
              {recs.investableMonthlyAmount > 0 && (
                <li className="flex gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-400">Automate your {formatRupees(recs.investableMonthlyAmount)} investable surplus into the target allocation shown above.</p>
                  </div>
                </li>
              )}
              <li className="flex gap-4">
                <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-400/20 text-xs font-bold text-emerald-400">{(!emergency.isSufficient ? 1 : 0) + (debt.warning ? 1 : 0) + (recs.investableMonthlyAmount > 0 ? 1 : 0) + 1}</div>
                <div>
